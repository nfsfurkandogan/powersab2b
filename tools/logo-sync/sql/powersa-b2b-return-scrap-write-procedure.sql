...
BEGIN
    ...
    SELECT
        ...
        0, 0, 0, 0, 0, 0,
        0, 4, @StockFicheRef, src.RowNo, COALESCE(@CustomerRef, 0), @Specode, CONVERT(FLOAT, src.Quantity),
        CONVERT(FLOAT, src.Price), CONVERT(FLOAT, src.LineTotal), 0, CONVERT(FLOAT, src.Price), 0, 1, 1, CONVERT(VARCHAR(251), src.LineExp),
        COALESCE(src.UomRef, 0), COALESCE(src.UsRef, 0), 1, 1, 0, CONVERT(FLOAT, src.VatRate), 0, CONVERT(FLOAT, src.LineTotal),
        0, 0, 0, CONVERT(FLOAT, src.LineTotal), MONTH(@ScrapDate), YEAR(@ScrapDate)
    FROM @Lines AS src
    ORDER BY src.RowNo;
