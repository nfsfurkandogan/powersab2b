...
"${SSH[@]}" "$REMOTE_USER@$REMOTE_HOST" "set -e; cd '$REMOTE_WEB_ROOT'; if [ -d '.next-prev-$STAMP/static' ]; then rsync -a --ignore-existing '.next-prev-$STAMP/static/' '.next/static/'; fi; rm -rf src src_upload_tmp .codex-staging ._.next .env.example .env.local.example README.md eslint.config.mjs next-env.d.ts postcss.config.mjs tsconfig.json components.json .gitignore; echo '$STAMP' > tmp/last-ui-deploy-stamp.txt; touch tmp/restart.txt; echo \"deployed_build=\$(cat .next/BUILD_ID)\"; echo \"fixed_icons=\$(find public/dashboard-icons/fixed -maxdepth 1 -type f -name '*.webp' 2>/dev/null | wc -l)\""
